...
const FormDonDangKy = (props: {
	record?: DonDangKy.IRecord;
	edit: boolean;
	visible: boolean;
	onClose: () => void;
	...
	return (
		<Modal
			...
			title={(props.edit ? 'Chỉnh sửa' : 'Thêm mới') + ' đơn đăng ký'}
		...
		>
			<Form form={form} layout="vertical" onFinish={onFinish}>
				<Form.Item name="hoTen" label="Họ tên" rules={[{ required: true, message: 'Vui lòng nhập họ tên' }]}>
					<Input placeholder="Họ tên ứng viên" />
				</Form.Item>
				...
				<Form.Item name="gioiTinh" label="Giới tính">
					<Select placeholder="Chọn giới tính" showSearch allowClear dropdownStyle={{ zIndex: 3000 }} filterOption={(input, option) =>
				...
				<Form.Item name="lyDoDangKy" label="Lý do đăng ký">
					<Input.TextArea rows={3} placeholder="Lý do đăng ký tham gia" />
				</Form.Item>
